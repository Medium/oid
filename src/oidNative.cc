...
Handle<Value> Id(const Arguments& args) {
    ...
    if (obj->IsNull()) {
        const char *err = "~~~ got null";
        Local<Value> exception = Exception::Error(String::New(err));
        ThrowException(exception);
        return Undefined();
    }

    int ident = obj->GetIdentityHash();
    return scope.Close(Number::New((double) ident));
}

void init(Handle<Object> target) {
    target->Set(String::NewSymbol("id"),
 ...
 */
void OidNative::InitClass(Handle<Object> target) {
    Local<String> className = String::NewSymbol("OidNative");
